...
def generate_base():
    print("Generating v4 LinkedIn Image Base via Imagen 4...")
    ...
    for model_name in models:
        try:
            ...
            # For models with 'predict' supported action, use models.predict
            ...
            # Check where the image is in the response. Typically in predictions[0].bytesBase64Encoded or similar
            if response.predictions:
                # Based on typical Vertex/Imagen response
                b64_data = response.predictions[0].get('bytesBase64Encoded')
                if b64_data:
                    image_bytes = base64.b64decode(b64_data)
                    image = Image.open(io.BytesIO(image_bytes))
                    image.save(output_base)
                    print(f"✅ Saved Base via {model_name}")
                    return True
