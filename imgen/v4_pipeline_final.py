...
def generate_base():
    ...
    try:
        ...
        if response.candidates:
            ...
            if part.inline_data:
                # The data is already bytes in the SDK response, but print showed b'\xff...'
                # In google-genai, part.inline_data.data is usually the bytes
...
def apply_v4_styling(input_path, output_path):
    ...
    offset = 12
    sdraw.text((x+offset, y+offset), text, fill=(0,0,0,250), font=font)
    ...
    # Arrow
    arrow_x = width // 2
    arrow_y_start = y + (bottom - top) + 60
    arrow_y_end = arrow_y_start + 150
    
    arrow_layer = Image.new('RGBA', img.size, (0, 0, 0, 0))
    adraw = ImageDraw.Draw(arrow_layer)
    adraw.line([(arrow_x, arrow_y_start), (arrow_x, arrow_y_end)], fill=font_color, width=35)
    ...
    asdraw.line([(arrow_x+6, arrow_y_start+6), (arrow_x+6, arrow_y_end+6)], fill=(0,0,0,200), width=35)
    asdraw.polygon([
